...
public class FFAGame extends Game {
    ...
    public void getScoreboardLines(Player player, LinkedList<String> lines) {
        if (state == GameState.WAITING) {
            lines.add("&6&l┃ &fPlayers&7: &6" + players.size() + "&7/&6" + getMaxPlayers());

            if (getVotedArena() != null) {
                lines.add("&6&l┃ &fMap&7: &6" + getVotedArena().getName());
            } else {
                lines.add("");
            ...
            lines.add("&6&l┃ &fRemaining&7: &6" + players.size() + "&7/&6" + getStartedWith());

            int potions = ItemUtils.countStacksMatching(player.getInventory().getContents(), ItemUtils.INSTANT_HEAL_POTION_PREDICATE);
            lines.add("&6&l┃ &fHealth Potions&7: &6" + potions);
            lines.add("&6&l┃ &fKills&7: &6" + kills.get(player.getUniqueId()));
        } else {
            if (winningPlayer == null) {
                lines.add("&6&l┃ &fWinner&7: &6None");
            } else {
                lines.add("&6&l┃ &fWinner&7: &6" + winningPlayer.getName());
            }
