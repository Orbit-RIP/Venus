...
public class GeneratorHandler {
	...
	@Getter
	private final List<ItemStack> generatorItems;
	...
	public GeneratorHandler() {
		...
		Bukkit.getScheduler().runTaskTimerAsynchronously(HCF.getInstance(), () -> {
			getGenerators().forEach(generator -> {
				...
				if (generator.getLastSpawn() + time < System.currentTimeMillis()) {
					new BukkitRunnable() {
						@Override
						public void run() {
							generator.spawnAbilities();
							generator.setLastSpawn(System.currentTimeMillis());
						}
					...
					new BukkitRunnable() {
						...
						public void run() {
							if (generator.getHologram() != null) {
								generator.getHologram().setLine(3, CC.translate("&7Spawn Time: " + TimeUtils.formatIntoMMSS((int) (((generator.getLastSpawn() + time) - System.currentTimeMillis()) / 1000))));
							}
						}
					}.runTask(HCF.getInstance());
				}
