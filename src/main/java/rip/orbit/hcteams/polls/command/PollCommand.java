...
public class PollCommand {
	...
	public static void pollsCreate(CommandSender sender, @cc.fyre.proton.command.param.Parameter(name = "yesCommand - = space") String yesCommand, @cc.fyre.proton.command.param.Parameter(name = "title") String title, @cc.fyre.proton.command.param.Parameter(name = "question", wildcard = true) String question) {
		...
		Bukkit.broadcastMessage(CC.translate(" "));
		Bukkit.broadcastMessage(CC.translate("&6&l┃ &fQuestion&7: &6" + poll.getQuestion()));
		Bukkit.broadcastMessage(CC.translate(" "));
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				if (selected.getNo() > selected.getYes()) {
					Bukkit.broadcastMessage(CC.translate("&6&l┃ &fMost Voted&7: &cNO"));
				} else if (selected.getNo() == selected.getYes()) {
					Bukkit.broadcastMessage(CC.translate("&6&l┃ &fMost Voted&7: &eTIE"));
				} else {
					Bukkit.broadcastMessage(CC.translate("&6&l┃ &fMost Voted&7: &aYES"));
				}
				Bukkit.broadcastMessage(CC.translate(" "));
