...
public class TeamLivesCommand {
    ...
    public static void livesAdd(Player sender, @cc.fyre.proton.command.param.Parameter(name = "lives") int lives) {
    ...
    public static void livesRevive(Player sender, @cc.fyre.proton.command.param.Parameter(name = "player") UUID whom) {
        Team team = HCF.getInstance().getTeamHandler().getTeam(sender);
        if( team == null ) {
            sender.sendMessage(ChatColor.RED + "You need a team to use this command.");
            return;
