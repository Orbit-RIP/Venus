...
public class TeamCaptainCommand {
    ...
    public static void captainAdd(Player sender, @cc.fyre.proton.command.param.Parameter(name = "player") UUID promote) {
    ...
    public static void captainRemove(Player sender, @cc.fyre.proton.command.param.Parameter(name = "player") UUID demote) {
        Team team = HCF.getInstance().getTeamHandler().getTeam(sender.getUniqueId());
        if( team == null ) {
            sender.sendMessage(ChatColor.RED + "You must be in a team to execute this command.");
            return;
