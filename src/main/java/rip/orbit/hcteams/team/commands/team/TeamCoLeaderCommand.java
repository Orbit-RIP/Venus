...
public class TeamCoLeaderCommand {
    ...
    public static void coleaderAdd(Player sender, @cc.fyre.proton.command.param.Parameter(name = "player") UUID promote) {
    ...
    public static void coleaderRemove(Player sender, @cc.fyre.proton.command.param.Parameter(name = "player") UUID demote) {
        Team team = HCF.getInstance().getTeamHandler().getTeam(sender.getUniqueId());
        if( team == null ) {
            sender.sendMessage(ChatColor.RED + "You must be in a team to execute this command.");
            return;
