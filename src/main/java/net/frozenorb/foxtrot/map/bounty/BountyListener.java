...
public class BountyListener implements Listener {
    ...
    public static void setBounty(@Parameter(name = "target") Player target) {
    ...
    private void newBounty() {
        ...
        Bukkit.getScheduler().runTaskLaterAsynchronously(Foxtrot.getInstance(), () -> {
            pickNewBounty();
        }, 30 * 20);
    }
    ...
    private void pickNewBounty() {
        ...
        if (suitablePlayers.isEmpty()) {
            Bukkit.getScheduler().runTaskLaterAsynchronously(Foxtrot.getInstance(), this::pickNewBounty, 20L);
            return;
        }
    ...
    private enum Reward {
        TWO_HUNDRED_FIFTY_DOLLARS("$250", player -> {
            Proton.getInstance().getEconomyHandler().deposit(player.getUniqueId(), 250);
        }),

        FIVE_HUNDRED_DOLLARS("$500", player -> {
            Proton.getInstance().getEconomyHandler().deposit(player.getUniqueId(), 500);
        }),

        SEVEN_HUNDRED_FIFTY_DOLLARS("$750", player -> {
            Proton.getInstance().getEconomyHandler().deposit(player.getUniqueId(), 750);
        }),
