...
public final class ServerFakeFreezeTask extends BukkitRunnable {
    ...
    private void freeze() {
        ...
        for (Player player : Foxtrot.getInstance().getServer().getOnlinePlayers()) {
            if (player.isOp()) {
                player.sendMessage(ChatColor.GOLD.toString() + ChatColor.BOLD + "Foxtrot autofreeze would have frozen the server.");
            }
        }
    ...
    private void unfreeze() {
        ...
        for (Player player : Foxtrot.getInstance().getServer().getOnlinePlayers()) {
            if (player.isOp()) {
                player.sendMessage(ChatColor.GOLD.toString() + ChatColor.BOLD + "Foxtrot autofreeze would have unfrozen the server.");
            }
        }
