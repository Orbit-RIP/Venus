...
public class EOTWCommand {
        ...
        if (Foxtrot.getInstance().getServerHandler().isEOTW()) {
            Bukkit.getScheduler().runTaskAsynchronously(Foxtrot.getInstance(), () -> {
                for (Team team : Foxtrot.getInstance().getTeamHandler().getTeams()) {
                    ...
                    System.out.println("Setting Team " + team.getName() + " raidable.");
                }
            });
    ...
    @Command(names={ "EOTW teleport" }, permission="foxtrot.eotw", hidden = true)
    ...
    @Command(names={ "tpall" }, permission="foxtrot.tpall", hidden = true)
    public static void TpAll(Player sender) {
        for (Player onlinePlayer : Foxtrot.getInstance().getServer().getOnlinePlayers()) {
            onlinePlayer.teleport(sender.getLocation());
        }

        sender.sendMessage(ChatColor.GOLD + "All online players teleported.");
    }

    @Command(names={ "EOTW pre" }, permission="foxtrot.eotw", hidden = true)
    ...
    @Command(names = {"eotw ffa"}, permission="foxtrot.eotw", hidden = true)
