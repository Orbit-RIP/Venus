...
public class EnchantmentLimiterListener implements Listener {
    ...
    public void onEntityDamageByEntity(EntityDamageByEntityEvent event) {
        if (event.getDamager() instanceof ExplosiveMinecart || event.getDamager() instanceof TNTPrimed) return;
        ...
        if (event.getDamager() instanceof Player && event.getEntity() instanceof Player) {
            Player player = (Player) event.getDamager();
            ItemStack hand = player.getItemInHand();
            if (DTRBitmask.CITADEL.appliesAt(player.getLocation()) || DTRBitmask.CONQUEST.appliesAt(player.getLocation()) || DTRBitmask.NO_KNOCKBACK.appliesAt(player.getLocation())) {
                String landAt = LandBoard.getInstance().getTeam(player.getLocation()).getName(player);
                if (hand.containsEnchantment(Enchantment.KNOCKBACK)) {
                    event.setCancelled(true);
                    player.sendMessage(CC.translate("&cYou may not use knockback items at " + landAt + "&c!"));
                }
            }
            ...
            if (Foxtrot.getInstance().getServerHandler().isEOTW()) {
                if (hand.containsEnchantment(Enchantment.KNOCKBACK)) {
                    event.setCancelled(true);
                    player.sendMessage(CC.translate("&cYou may not use knockback items while &4EOTW &cis active!"));
                }
            }
        }
    }
