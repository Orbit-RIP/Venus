...
public class ChatListener implements Listener {
    ...
    public void onAsyncPlayerChat(AsyncPlayerChatEvent event) {
        ...
        Profile user = Nebula.getInstance().getProfileHandler().fromUuid(event.getPlayer().getUniqueId());
        Team playerTeam = Foxtrot.getInstance().getTeamHandler().getTeam(event.getPlayer());
        String rankPrefix = user.getActiveRank().getPrefix();
//        String customPrefix = User.getByUUID(event.getPlayer().getUniqueId()).getTag().getPrefix() + " ";
        ChatMode playerChatMode = Foxtrot.getInstance().getChatModeMap().getChatMode(event.getPlayer().getUniqueId());
        ChatMode forcedChatMode = ChatMode.findFromForcedPrefix(event.getMessage().charAt(0));
        ...
        switch (finalChatMode) {
            case PUBLIC:
                ...
                String publicChatFormat = FoxConstants.publicChatFormat(event.getPlayer(), playerTeam, rankPrefix);
                ...
                for (Player player : event.getRecipients()) {
                    if (playerTeam == null) {
                        ...
                        publicChatFormat = FoxConstants.publicChatFormat(event.getPlayer(), player, playerTeam, rankPrefix);
                        finalMessage = String.format(publicChatFormat, event.getMessage());
                ...
                publicMessagesSent.incrementAndGet();
    ...
    public void onPlayerjoin(PlayerJoinEvent event) {
        ...
        for (String message : Foxtrot.getInstance().getConfig().getStringList("join_message")){
